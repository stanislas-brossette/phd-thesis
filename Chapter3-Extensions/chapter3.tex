...
In Section~\ref{sec:contact_constraints}, we defined a set of constraints (equations~\Eqref{eq:coplanarity}) that ensures the coplanarity of two planar surfaces in geometric planar contact.
...
In this section, we propose a simple way to formulate geometric contact formation to have an arbitrary intersection shape in a robotic (humanoid) posture generation problem, when searching for a mobile contact between two surfaces.
...
A simple example is when a human climbs stairs. He usually contacts only the front half of its foot with each step, the heel almost never touches the steps.
...
In this section, we propose to apply the methods of lifted optimization presented in~\cite{Albersmeyer:2010:LNM:1958447.1958472} to posture generation problems.

The idea of lifting optimization is to consider a complicated equation $F(q) = 0$ and decomposing it into simpler functions $\{f_1, f_2, \ldots, f_m\}$ that all depend only on the original variable $q$ and on the output of functions of lower index~\ref{eq:simplerFunctions} that are replaced by additional variables $\{w_1, w_2, \ldots, w_m\}$, that we call the `lifted variables'.
...
By reducing the complexity of the functions, which often comes down to reducing their polynomial degree, we ought to improve the quality of their quadratic approximations, which should lead to better convergence properties of the Newton scheme.
...
We devised a set of simple robots to be the basis of our experimentation.
