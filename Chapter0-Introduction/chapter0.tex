...
The road towards creating such performant robots is still long, but in the recent years, we have witnessed a wide growth of the number of robots used in many environments and of the variety of tasks that they are entrusted with.
...
This competition brought together robotics groups from Universities, laboratories, and private companies to work on a common goal: develop a partially teleoperated robot system that can realize several tasks without any human physical intervention.
The robots were required to drive a car, open a door, climb stairs, cross debris, drill a hole in a wall, etc.
All these tasks can usually be broken down into sets of elementary tasks in human language, such as `put a hand in contact with target', `put a foot on next step', `avoid collision with that object', `maintain stability' or `look in that direction'.
...
All the joints of the robot must be piloted separately and in coordination in order to make the whole robot complete the tasks at hand.
...
Given a set of tasks specified by the user for a robot to satisfy, the questions that immediatly comes to mind is: "Is that set of tasks feasible?".
...
The formulation and resolution of posture generation problems are the missions of the `Posture Generator', and the development of this tool is the central topic of this dissertation.
...
Through this thesis, we develop a nonlinear constrained optimization solver on manifolds, that we believe to be the first ever developed, and use it to solve optimization problems on their native search manifolds.
