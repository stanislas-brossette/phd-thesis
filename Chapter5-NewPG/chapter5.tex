...
\item The Frame constraint is a generic type of constrain which consists in blocking a predefined set of degrees of freedom of one frame $F_2$ w.r.t. another frame $F_1$.
...
In the event that the two frames do not have a satisfying configuration to write the desired constraint with this formalism (for example, if one wants to write a planar contact between two surfaces $S_1$ and $S_2$, where $\vec{z_1}$ and $\vec{z_2}$ are outbound normal vectors, then equation~\ref{eq:2_rotations_blocked} is not satisfactory because we want to oppose $\vec{z_1}$ and $\vec{z_2}$, not identify them). Then the user defines a new frame $F_2'$ that is a constant 3D transformation away from $F_2$ (in that case a rotation of $\pi$ around $(O_1,\vec{x_1})$ would suffice).
...
In this section, we present some scenarios that are build and solved with our posture generator and solver.
...
The result generated by our {\tt PGSolver} is depicted in Figure~\ref{fig:comanoid}
\begin{figure}
  \centering
...
The goal is to be able the reach the four corners of a panel of circuit breakers (in green on Figure~\ref{fig:getafe}) to activate them.
...
We parametrize this shape as shown in Figure~\ref{fig:param_wrist_detail}:
...
The equations are given in Figure~\ref{fig:param_wrist_detail}.
...
The bottom row of Figure~\ref{fig:contact_plan_sphere} displays the results of this simulation for the robot pointing in 4 directions.
...
In Figure~\ref{fig:hrp4_cube} we present some results for a posture generation problem with manipulation: On the left side, the feet are free to move on a sphere, and, on the right side, the left foot position is fixed and the right foot is free to move on the ground.
...
In Figure~\ref{fig:CCS}, we show the Catmull-Clark surface (in blue) corresponding to the mesh (in green) of an HRP-2 link, for two different levels of smoothness.
...
The second example (Figure~\ref{fig:examples_CCS}, right) demonstrates our PG applied to grasping an object.
...
The third example (Figure~\ref{fig:HRP4_cubes}) involves a stack of cubes on which the robot must stand, using its hands and feet to maintain its stability and respect its torque limits.
...
For collisions, we model the stack of cubes with a set of 21 cubes all slightly smaller than the actual cubes see Figure~\ref{fig:HRP4_cubes} (Right), and we require the robot to avoid collision with those cubes.
