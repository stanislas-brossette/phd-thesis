...
The main goal of this thesis is to extend the state of the art of posture generation by in one hand improve the range of problems that it can solve through extending its formulation and on the other hand, improve and adapt the methods used to solve those problems.
This section focuses on the later.
We formulate posture generation problems in an optimization friendly way in order to solve them using non-linear constrained optimization routines.
In the following, we propose to apply the methods presented in~\cite{Albersmeyer:2010:LNM:1958447.1958472} to our posture generation problems.
...
One obvious drawback of this approach for solving a non-linear problem is the increased size of the approached problem to solve at each iteration.
...
With good intuition, the author of~\cite{Albersmeyer:2010:LNM:1958447.1958472} states that the lifting idea could be directly transfered in the context of kinematic chain arising in robotics.
Indeed, as we stated in Section~\ref{sub:forward_kinematics} the equations that governs the transformation of a robot's end effector on body $i$ ${}^i X_W(q)$ is constructed iteratively and seems like a good candidate for lifting.
...
The transformation of a body is used in almost all the constraints and cost functions of posture generation problems, thus we could use this lifting approach on almost all of them.
...
To do so, we used symbolic variables, a symbolic equation can be seen as a graph of atomic operations (addition, multiplication and other functions), which can in turn be explored and cut at appropriate places where we can insert our additional variables.
...
\begin{figure}
  \centering
  \includegraphics[width=0.55\textwidth]{graphDirectEEPos.png}
  \caption{Computation graph for end effector position of a 2 axis planar robotic arm}
\label{fig:graph_direct}
\end{figure}

To lift our equations, we explore the graph from left to right, and at each node of the graph, we consider the sub-graph that lies on its left and if this sub-graph violates our criterion, it is lifted, a lifting variable is added and replaces the sub-graph.
Then we start again and explore the graph from left to right.
Given that the equations that we want to solve are polynomials of trigonometric functions, we decided to lift them based on the two following criteria (other criteria would be acceptable as well):
\begin{itemize}
  \item Maximum degree of the lifted polynomial.
  \item Eliminating or not the trigonometric functions.
\end{itemize}
We define the degree function as the degree of a polynomial with the difference that trigonometric functions are considered to be polynomials of infinite degree (And thus are always lifted, no matter the maximum degree).
By eliminating the trigonometric functions, we mean that we reformulate the problem so that, given a variable $x$ that appears in our equations in the forms $\cos(x)$ or $\sin(x)$, we introduce 2 new variables $s_x$ and $c_x$, replace all the occurrences of $\cos(x)$ by $c_x$ and all the occurrences of $\sin(x)$ by $s_x$, and to ensure the equivalence of the two formulations, we add the equation ${c_x}^2 + {s_x}^2=1$ to our problem.

For example, if we lift the set of equation~\ref{eq:direct_position} with a maximum degree of 2 and without eliminating the trigonometric functions we get the following system, with all the $w_{i_j}$ being the additional lifted variables:
\begin{equation}
  \begin{array}{c}
...
In this study, we focused on fixed base robots with only revolute joints.
Also, to make it simpler to study the influence of the number of degrees of freedom of a robot on the optimization performance, we mostly studied planar robots composed of $n$ successive links of length 1 attached to one another by revolute joints.
We also studied a 7 axis 3D arm robot and a torso robot with 2 arms.
Illustration of those different types of robots are given in figure~\ref{fig:robots}
\begin{figure*}
  \centering
