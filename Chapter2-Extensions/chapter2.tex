...
The main goal of this thesis is to extend the state of the art of posture generation by in one hand improving the formulation of problems to enable describing more complex and various problems in simpler ways and on the other hand, improve and adapt the resolution methods for those problems.
...
In the following, we propose to apply the methods of lifted optimization presented in~\cite{Albersmeyer:2010:LNM:1958447.1958472} to our posture generation problems.
...
With this formulation and the additional variables $x_i,\ i\in[1,\ldots, m]$, the problem $F(u) = 0$ can be rewritten as the lifted problem:
...
Fortunately,~\cite{Albersmeyer:2010:LNM:1958447.1958472} proposes an algorithmic trick leveraging the triangularity of the jacobian of $\mathbf{G}(u,x)$ to condense the lifted problem to have the same size as the original one.
Thus the resolution of each iteration should take approximately the same time as with the original problem.

The lifted variable approach is known to be superior to the non-lifted in the problem of shooting methods for boundary value problems~\cite{osborne:1969:shooting}.
With good intuition, the authors of~\cite{Albersmeyer:2010:LNM:1958447.1958472} state that the lifting idea could be directly transfered in the context of kinematic chain arising in robotics.
Indeed, as we state in Section~\ref{sub:forward_kinematics} the equations that governs the transformation of a robot's end effector on body $i$, ${}^i X_W(q)$ is constructed iteratively and seems to be a good candidate for lifting.
If we considere a single chain robot with $m$ bodies, such that $\kappa (m) = \{0, 1, 2, \ldots m\}$ the constraint ${}^m X_W(q) = X_\text{goal}$ can naturally be lifted as follows:
...
In order to explore the capabilities of optimization on lifted problems, we developped and studied several lifting approaches.
...
The operators `addition', `substraction', `multiplication', `sinus' and `cosinus' are enough to describe the kinematics of a robot.

We developped a generic lifting algorithm that allows to automatically lift polynomials of trigonometric functions based on a given criterion.
...
Figure~\ref{fig:graph_direct} presents the graph of atomic operations for equation~\ref{eq:direct_position}, where the green circles represent the variables, the pink ones represent the atomic operations and the red circles represent the final atomic operations that leads to the final expressions.
\begin{figure}
  \centering
...
To lift a set of equations, we explore the graph from left to right. At each node of the graph, we consider the sub-graph that lies on its left and if this sub-graph violates our criterion, it is lifted, a lifting variable is added and replaces the sub-graph.
Then we iterate this process on the remaining graph until it does not violate the criterion.
Given that the equations to solve are polynomials of trigonometric functions, we decided to lift them based on a criterion of the maximum degree of the lifted polynomials.
We define the degree function as the degree of a polynomial with the difference that trigonometric functions are considered to be polynomials of infinite degree (And thus are always lifted, no matter the maximum degree criterion).
We also considere the option of eliminating or not the trigonometric functions.
...
For example, lifting the set of equation~\ref{eq:direct_position} with a maximum degree of 2 and without eliminating the trigonometric functions gives the following system, with all the $w_{i_j}$ being the additional lifted variables:
\begin{equation}
  \begin{array}{c}
...
We also developped a lifting method that follows the principle described in~\ref{eq:robot_lift}, in which case, we get another set of equations similar to~\ref{eq:lifted_equations}.

\subsection{Optimization on lifted variables}
\label{subsec:optimization_on_lifted_variables}

We want to estimate the gain of performances yielded by the use of a lifted formulation of a posture generation problem.
...
The only difference is that with the lifted full-space problem (non condensed case) each iteration takes more time to compute.
In our study, this is not a problem, we want to evaluate the performances in terms of number of iterations first, and if it shows that the lifted problem resolution outperforms the non lifted, then we will implement the condensation trick for our problems and solvers which will bring the per iteration time to approximately the same as in the direct problem case.
Therefore, we only tried those algorithms on the direct problems (non lifted problem) and the lifted full-space problems.

In addition to the matlab solvers, we implemented the optimization algorithms proposed in~\cite{Albersmeyer:2010:LNM:1958447.1958472}:
\begin{itemize}
  \item Lifted Newton
  \item Lifter Gauss-Newton
  \item Lifted SQP
\end{itemize}
...
We implemented the lifted and direct SQP approach as described in paragraph 3.2.3 of~\cite{Albersmeyer:2010:LNM:1958447.1958472}.
The hessian matrices in our problems are usually not symmetric positive definite, so we need to use a globalization method as well as a regularisation of the Hessian to enforce convergence.
For globalization, we implemented some line search methods with Armijo and Wolfe conditions, as well as a filter with backtracking line search. Those method are presented in~\cite{bonnans:springer:2002}.
And for regularization, we approximate the full-space hessian matrix through BFGS updates (with Powell's correction) or SR1 updates as presented in~\cite{bonnans:springer:2002}.
...
In order to evaluate the performances of the lifting approach in the resolution of posture generation problems, we realized many experimentations of solving simple inverse kinematics problems with the different solvers and lifting methods available.

A difficulty that arises in that endeavour lays in the inherent combinatorial aspect on the choice of formulation and resolution method.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Lifted based on Joint
  \end{itemize}
  \item Treatment of trigononetric equations
  \begin{itemize}
...
Illustration of those different types of robots are given in figure~\ref{fig:robots}, where blue lines represent the links, red circles represent the revolute joints, green square represent the origin of links, the biggest green square represents the origin of the base link of the robot, and the red cross is the end effector.
\begin{figure*}
  \centering
...
We studied many of the possible configurations, but not all of them.
Still, it seems possible to draw some conclusions out of it.

Since we do not use the condensation trick, we only focused on getting the lowest number of iterations for a resolution, the highest rate of successful optimizations and did not focus much on their duration.
From here on, an experiment in which we made a combination of choice and during which the problem is solved numerous times for different initial points and/or goals to study the performances of the solver will be called a "bench".
We gathered the results of our benches in terms of average number of iterations per optimization in the annexed document.
To get an idea of the influence of the dimensionality of the problem on the performances of the solver, we made benches on several planar robots with successive links as described in Section ?? with number of links going from 2 to 12.
Unfortunately, in most cases, the direct approach outperforms the lifted one.
The only case where we observed a neat advantage of the lifted methods compared to the direct one was when using the interior point method that is built-in in fmincon in Matlab with a randomized initialization and the norm 2 of the articular parameters vector as a cost function.
But then, the direct method was showing very bad performances and even though the lifted was better here, the average number of iterations before convergence was much higher than with other optimization method (up to twice more than for most other solvers).

The overall results we extracted from those bench is that the most performing method is: Solving the direct problem, using BFGS regularization and either Armijo or filter line searches, with random initial lifted variables and no cost function.

We also started doing some benches with an initial guesses close to the solution. We observed that when the lifted variables are initialized randomly the direct problem is solved faster. And when the lifted variables are initialized so that the lifted equations are satisfied, the results are sensibly the same for lifted and direct approaches.
