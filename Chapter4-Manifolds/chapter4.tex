...
Posture generation has been traditionally formulated as a problem over the Euclidean space.
Robots variable may, however, be expressed over non-Euclidean manifolds.
...
As such, it can be parametrized \emph{locally} by $3$ variables, for example, a choice of Euler angles. But any such parametrization necessarily exhibits singularities when taken as a global map (e.g.\ gimbal lock for Euler angles), which can be detrimental to the optimization process.
...
If further the gradient is needed, that projection must also be accounted for (authors in~\cite{bouyarmane:humanoids:2012} explained this issue in great details for robotics problems with free-floating basis).
...
Having intermediate values remaining on the manifold can be useful to evaluate additional functions that pre-suppose it (additional constraints, external monitoring, etc.).
...
To the best of our knowledge, constrained optimization on manifolds has drawn few research for now.
...
So another approach is to be taken.
...
\end{align}
$\frac{\partial f'}{\partial y}$ denotes the gradient of $f'$ with respect to an element of $\mathbb{E}$, which is the derivative that is usually computed for the use in classical optimization schemes.
...
In such cases, we approximate the second order derivative by comparing first order information (tangent vectors) taken on distinct points of the manifold.
...
During the optimization process, we need the expression of the derivatives of $\phi_x$ in order to compute the gradients of the cost function and the constraints at the beginning of each iteration.
...
In some cases, this quantity is invariant w.r.t $x$ and can be computed once for all.
...
\end{equation*}
The pseudolog operator `moves' the vector $T_x\mathcal{M}$ from $x$ to $y$.
...
The Manifold class describes the abstract mathematical structure of a non-Euclidean manifold and defines a common interface for all elementary manifolds to implement (retractation, pseudoLog...).
...
This leads to trust region methods that we favor over line-search approaches.
...
Finally, the constraints related to the trust-region added to the problem is to:
\begin{equation}
...
Dropping the distinction between Linear and Non-Linear Constraints,
...
Aside from the manifold adaptation, our main departure from Fletcher is in the Hessian computation where we used an approximation because the exact one is too expensive to compute in our problems.
...
For all these reasons, this update method is very attractive.
...
Although those problems are all on $\mathbb{R}^n$, thus, do not include the non-Euclidean implementations of our solver, it is beneficial to compare its performances with other solvers.
