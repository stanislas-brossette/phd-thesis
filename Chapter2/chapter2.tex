...
$f$ is the objective function. And $c_i$ are the constraint funcitons.
...
And finding the global minimizer of $f$ in $\Omega$ is a difficult problem that we donot treat in this thesis. 
Our goal is to find a local minimizer $x^*$ of $f$ in $\Omega$.

\paragraph{The first order optimality condition (Karush-Kuhn-Tucker Condition)}
The Karush-Kuhn-Tucker(KKT) condition is a necessary condition verified by all solutions of problem \ref{formulation_NLCP}. 
...
Most optimzation algorithms make the assumption that at the solution, the constraints satisfy the Linear Independence Constraints Qualification (LICQ)
