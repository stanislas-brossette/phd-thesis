...
To find it, the QP will start from a random active set $\mathit{A}_0$ ignore the non-active constraints and try to solve the QP with the remaining active constraints, then if the solution is not satisfactory, it will change the active set and iterate over this process until the optimal active set and solution is found for the QP \ref{IQP}.
...
In the SPQ algorith, it is necessary to have access to the Hessian of the Lagrangian $\nabla_{xx}^2\mathcal{L}$ to be able to devise the QP subproblem to solve.
For some solving strategies like the Line-Search, it is necessary that $\nabla_{xx}^2\mathcal{L}$ is definite positive.
...
Since we are following an iterative process, it is not necessary to have an exact knowledge of the Hessian and using an approximation of it is often enough.
