...
To generate problems that are known to be feasible, we start by computing the pose of 4 frames that the robot can reach simultaneously with its right foot, left foot, right gripper and left gripper(that we denote respectively $F_\text{rightFoot}$, $F_\text{leftFoot}$, $F_\text{rightGripper}$, $F_\text{leftGripper}$), by querrying the Direct Kinematics algorithm for a random configuration $q_\text{rand}$ of the robot that is within its joint limits.
...
In particular, we require the contacts on the foot to be fixed unilateral friction contacts (with the forces in the friction cones) and the contacts on the ones on the hands to be fixed bilateral contacts (where the forces can be in applied in any direction.
...
Using this process, we compute three types of problems with different degrees of constraints:
\begin{itemize}
  \item 2 Contacts: only the foot are in contact, the hands are free of constraints.
  \item 3 Contacts: both foot and the right gripper are in contact, the left hands is free.
...
Some initial testings where we solved the recorded problems with 4 contacts with the half-sitting configuration $q_\text{half-sitting}$ as initial guess shows that a solution is found for only 35\% of the problems.
...
We ran another set of tests in which the value of $q_\text{rand}$ is taken closer to the half-sitting, to do so, we simply take the average between the random configuration and the half-sitting:
\begin{equation}
...
The initial distance between for a problem where $q_0$ is the initial guess and $q^*$ is a solution is defined as: $d=\|q_0-q^*\|^2$.
...
We sample the range of initial distances into 20 segments of same length and report the average results on each segment as a data point in figure~\ref{fig:evalPG}.
...
The results obtained show that, unlike what we expected, there is not one Hessian update method that is much better than all others in every case.
The BFGS and SR1 approaches with individual or grouped approaches present similar and consistent behaviors on the 3 types of problems.
...
This experiment shows that the closer the initial guess is to the solution, the more likely the solver is to converge to a solution and gives us a quantitative estimate of the expected success rate with respect to the initial distance.
Overall, solving those problems with our solver using the BFGS method on individual Hessians seems to be the safest choice as it has consistently good success rates and convergence times on the 3 types of problems.
...
Perhars choosing automatically the update method based on the structure of the problem at hand would allow to increase the general performance of our solver.
...
In this section, we present our approach to generate multi-contact planning scenarios in a sensory acquired environment. This work was done prior to the development of our posture generator and therefore make use of a previous version of it that is presented in~\cite{bouyarmane:humanoids:2010}.
...
%The goal is to real a pannel of circuit breakers (in green on figure~\ref{fig:getafe}) to activate them.
