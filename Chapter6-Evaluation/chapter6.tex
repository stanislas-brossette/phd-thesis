...
Our goal here is to generate postures with constraints that describe contacts between the robot and the acquired point cloud that represent the environment.
...
A similar formulation could be used in posture generation to smoothen the contact constraints when a point cloud is involved.
...
It would serve our purpose to devise a criterion that guarantees that for all imprecision of value less than a given $\delta$ on the location of the element to contact with, there exists a solution configuration in which the contact with $P$ is maintained.
But such criterion is not straightforward to devise, and as of now, we have not found any promising leads, except using a space-grid discretisation of the possible uncertainty and generating a solution posture for each point of the grid.
