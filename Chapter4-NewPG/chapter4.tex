...
The code of the dot function that translates eq:~\ref{eq:dot} is pretty straightforward:
\begin{center}
...
For example, for a planar contact, we can write the constraints~\ref{eq:planar_contact_bad}, with $\vec{z_1}$ and $\vec{z_2}$ the outbound normals to the surfaces in contact.
\begin{equation}
  \left\{
  \begin{array}{ll}
...
When the two surfaces are plan, a contact between them can be written simply with the set of equations~\ref{eq:planar_contact}.
...
When it comes to non-flat surfaces, the location of the frames used in equations~\ref{eq:planar_contact} matters, as the quantities in the constraint's equations depend on it.
